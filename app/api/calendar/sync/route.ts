  ...
  try {
    ...
    if (calendarResponse.status === 401 && profile.calendar_refresh_token) {
      ...
      if (newToken) {
        ...
        await db.collection("profiles").doc(id).update({ calendar_access_token: newToken }).eq("id", user.id)
    ...
    const { error } = await db.collection("calendar_events").upsert(eventsToInsert, {
