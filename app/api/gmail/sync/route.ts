...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.log("[v0] Profile fetch result:", { hasProfile: !!profile })
    ...
    if (gmailResponse.status === 401 && profile.gmail_refresh_token) {
      ...
      try {
        ...
        if (newToken) {
          ...
          console.log("[v0] Token updated successfully")
    ...
    // Batch write emails to Firestore
    const batch = db.batch()
    for (const email of emailsToInsert) {
      const emailRef = db.collection("emails").doc(email.gmail_id)
      batch.set(emailRef, email, { merge: true })
    }
    await batch.commit()
    
    console.log("[v0] Emails upserted successfully")
