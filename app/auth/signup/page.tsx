...
export default function SignUpPage() {
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { getAuth, getDb, createUserWithEmailAndPassword, doc, setDoc } = await import("@/lib/firebase/client")
      const auth = getAuth()
      const db = getDb()
      ...
      if (user) {
        console.log("[v0] User created, creating profile with ID:", user.uid)
